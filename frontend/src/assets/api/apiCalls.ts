...
export const purchaseProductByEntity = async ({ productInstanceId, productPrice, certificationPrice, buyerId, oldOwnerId }: { productInstanceId: number, productPrice: number, certificationPrice: number, buyerId: number, oldOwnerId: number }): Promise<AxiosResponse<{ data: { message: string } }>> => {
	try {
		const currentRole = await getEntityRole()
